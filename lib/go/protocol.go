...
func (f *FProtocol) ReadRequestHeader() (*FContext, error) {
	...
	if !ok {
		return nil, NewFProtocolExceptionWithType(thrift.INVALID_DATA, errors.New("frugal: request missing op id"))
	}
...
func (f *FProtocol) writeHeader(headers map[string]string) error {
	...
	if n, err := f.Transport().Write(buff); err != nil {
		...
		return NewFProtocolExceptionWithType(thrift.UNKNOWN_PROTOCOL_EXCEPTION, errors.New("frugal: failed to write complete protocol headers"))
	}
...
func readHeader(reader io.Reader) (map[string]string, error) {
	...
	if buff[0] != protocolV0 {
		return nil, NewFProtocolExceptionWithType(thrift.BAD_VERSION, errors.New(fmt.Sprintf("frugal: unsupported protocol version %d", buff[0])))
	}
...
func getHeadersFromFrame(frame []byte) (map[string]string, error) {
	if frame[0] != protocolV0 {
		return nil, NewFProtocolExceptionWithType(thrift.BAD_VERSION, errors.New(fmt.Sprint("frugal: unsupported protocol version %d", frame[0])))
	}
...
func readHeadersFromReader(reader io.Reader, size int32) (map[string]string, error) {
	...
	for i := int32(0); i < size; {
		...
		if i > size || i+nameSize > size {
			return nil, NewFProtocolExceptionWithType(thrift.INVALID_DATA, errors.New("frugal: invalid protocol header name"))
		}
		...
		if i > size || i+valueSize > size {
			return nil, NewFProtocolExceptionWithType(thrift.INVALID_DATA, errors.New("frugal: invalid protocol header value"))
		}
