...
// NewNATSServer returns a FServer which uses the NATS messaging system
...
func NewNATSServerFactory9(
	...
	outputTransportFactory thrift.TTransportFactory,
	inputProtocolFactory FProtocolFactory,
	outputProtocolFactory FProtocolFactory) FServer {
...
func (n *natsServer) ProcessorFactory() FProcessorFactory {
...
func (n *natsServer) InputProtocolFactory() FProtocolFactory {
	return n.inputProtocolFactory
}

func (n *natsServer) OutputProtocolFactory() FProtocolFactory {
...
func (n *natsServer) AcceptLoop() error {
	sub, err := n.conn.QueueSubscribe(n.subject, queue, func(msg *nats.Msg) {
		if msg.Reply != "" {
			...
			if err != nil {
				log.Println("frugal_nats: error accepting client transport:", err)
				return
			}
			...
			if err := n.conn.PublishRequest(msg.Reply, listenTo, []byte(connectMsg)); err != nil {
				log.Println("frugal_nats: error publishing transport inbox:", err)
				if n.isHeartbeating() {
					n.remove(heartbeat)
				}
			...
			log.Printf("frugal_nats: discarding invalid connect message %+v", msg)
		}
	})
	if err != nil {
		return err
	...
	log.Println("frugal_nats: server running...")
	<-n.quit
...
func (n *natsServer) acceptHeartbeat(heartbeat string) {
	...
	if err != nil {
		log.Println("frugal_nats: error subscribing to heartbeat", heartbeat)
		return
	}
	...
	for {
		...
		case <-time.After(time.Duration(n.heartbeatDeadline)):
			...
			if missed >= maxMissedHeartbeats {
				log.Println("frugal_nats: client heartbeat expired")
				n.remove(heartbeat)
				sub.Unsubscribe()
				return
			}
...
func (n *natsServer) accept(listenTo, replyTo, heartbeat string) (thrift.TTransport, error) {
	...
	go func() {
		if err := n.processRequests(client); err != nil {
			log.Println("frugal_nats: error processing request:", err)
		}
		n.remove(heartbeat)
	}()
	return client, nil
