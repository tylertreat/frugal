...
func (g *Generator) addToPubspec(p *parser.Program, dir string) error {
	// TODO: Possibly just write a new pubspec from scratch
	pubFilePath := filepath.Join(dir, "pubspec.yaml")
	pubFile, err := os.OpenFile(pubFilePath, syscall.O_RDWR, 0777)
	defer pubFile.Close()
	if err != nil {
		return err
	}
	stat, err := pubFile.Stat()
	if err != nil {
		return err
	}
	bts := make([]byte, stat.Size())
	if _, err := pubFile.Read(bts); err != nil {
		return err
	}

	pub := make(map[interface{}]interface{})
	if err := yaml.Unmarshal(bts, &pub); err != nil {
		return err
	}

	// add frugal depdency
	depsMap, ok := pub["dependencies"]
	if !ok {
		return fmt.Errorf("frugal: pub file missing dependencies")
	}
	// TODO: Change this path to pub or git reference
	frugalMap := map[interface{}]interface{}{"path": "../../../../lib/dart"}
	depsMap.(map[interface{}]interface{})["frugal"] = frugalMap
	pub["dependencies"] = depsMap

	// rewrite description
	pub["description"] = "Autogenerated by frugal compiler"
	d, err := yaml.Marshal(&pub)
	if err != nil {
		return err
	}
...
func (g *Generator) exportClasses(p *parser.Program, dir string) error {
	...
	for _, scope := range p.Scopes {
		exports += fmt.Sprintf("export 'src/%s%s.%s' show %sPublisher, %sSubscriber;\n",
			lib_prefix, strings.ToLower(scope.Name), lang, scope.Name, scope.Name)
	}
