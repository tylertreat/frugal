...
class MyScopeDurablePublisher {
  frugal.FDurablePublisherTransport transport;
  frugal.FProtocolFactory protocolFactory;
  Map<String, frugal.FMethod> _methods;
  MyScopeDurablePublisher(frugal.FDurableScopeProvider provider, [List<frugal.Middleware> middleware]) {
    transport = provider.publisherTransportFactory.getTransport();
    protocolFactory = provider.protocolFactory;
    var combined = middleware ?? [];
    combined.addAll(provider.middleware);
  ...
  Future publishnewItem(frugal.FContext ctx, String groupId, t_vendor_namespace.Item req) {
    return this._methods['newItem']([ctx, groupId, req]);
  }

  Future _publishnewItem(frugal.FContext ctx, String groupId, t_vendor_namespace.Item req) async {
    var op = "newItem";
    var prefix = "";
    var topic = "${prefix}MyScope${delimiter}${op}";
    var memoryBuffer = new frugal.TMemoryOutputBuffer(transport.publishSizeLimit);
...
class MyScopeDurableSubscriber {
  final frugal.FDurableScopeProvider provider;
  final List<frugal.Middleware> _middleware;

  MyScopeDurableSubscriber(this.provider, [List<frugal.Middleware> middleware])
      : this._middleware = middleware ?? [] {
    this._middleware.addAll(provider.middleware);
}

  Future<frugal.FSubscription> subscribenewItem(dynamic onItem(frugal.FContext ctx, String groupId, t_vendor_namespace.Item req)) async {
    var op = "newItem";
    var prefix = "";
    var topic = "${prefix}MyScope${delimiter}${op}";
    var transport = provider.subscriberTransportFactory.getTransport();
  ...
  frugal.FDurableAsyncCallback _recvnewItem(String op, frugal.FProtocolFactory protocolFactory, dynamic onItem(frugal.FContext ctx, String groupId, t_vendor_namespace.Item req)) {
    frugal.FMethod method = new frugal.FMethod(onItem, 'MyScope', 'subscribeItem', this._middleware);
    callbacknewItem(thrift.TTransport transport, String groupId) {
      var iprot = protocolFactory.getProtocol(transport);
      var ctx = iprot.readRequestHeader();
      var tMsg = iprot.readMessageBegin();
      if (tMsg.name != op) {
