...
class Client(Iface):
    ...
    async def _buyAlbum(self, ctx, ASIN, acct):
        ...
        try:
            ...
            raise FTimeoutException('buyAlbum timed out after {} milliseconds'.format(ctx.get_timeout()))
        finally:
    ...
    async def _enterAlbumGiveaway(self, ctx, email, name):
        ...
        try:
            ...
            raise FTimeoutException('enterAlbumGiveaway timed out after {} milliseconds'.format(ctx.get_timeout()))
        finally:
            await self._transport.unregister(ctx)
        return result
