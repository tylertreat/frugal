...
class _buyAlbum(FProcessorFunction):
    ...
    async def process(self, ctx, iprot, oprot):
        ...
        try:
            ...
            async with self._lock:
                e = _write_application_exception(ctx, oprot, "buyAlbum", ex_code=TApplicationException.UNKNOWN, message=e.args[0])
            raise e from None
        async with self._lock:
            try:
...
class _enterAlbumGiveaway(FProcessorFunction):
    ...
    async def process(self, ctx, iprot, oprot):
        ...
        try:
            ...
            async with self._lock:
                e = _write_application_exception(ctx, oprot, "enterAlbumGiveaway", ex_code=TApplicationException.UNKNOWN, message=e.args[0])
            raise e from None
        async with self._lock:
            try:
                oprot.write_response_headers(ctx)
