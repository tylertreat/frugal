...
class AlbumWinnersDurablePublisher(object):
    ...
    @gen.coroutine
    def publish_ContestStart(self, ctx, group_id, req):
        """
        Args:
            ctx: FContext
            group_id: string
            req: list
        """
        yield self._methods['publish_ContestStart']([ctx, group_id, req])

    @gen.coroutine
    def _publish_ContestStart(self, ctx, group_id, req):
        op = 'ContestStart'
        prefix = 'v1.music.'
    ...
    @gen.coroutine
    def publish_TimeLeft(self, ctx, group_id, req):
        """
        Args:
            ctx: FContext
            group_id: string
            req: Minutes
        """
        yield self._methods['publish_TimeLeft']([ctx, group_id, req])

    @gen.coroutine
    def _publish_TimeLeft(self, ctx, group_id, req):
        op = 'TimeLeft'
        prefix = 'v1.music.'
        topic = '{}AlbumWinners{}{}'.format(prefix, self._DELIMITER, op)
        buffer = TMemoryOutputBuffer(self._transport.get_publish_size_limit())
    ...
    @gen.coroutine
    def publish_Winner(self, ctx, group_id, req):
        """
        Args:
            ctx: FContext
            group_id: string
            req: Album
        """
        yield self._methods['publish_Winner']([ctx, group_id, req])

    @gen.coroutine
    def _publish_Winner(self, ctx, group_id, req):
        op = 'Winner'
        prefix = 'v1.music.'
        topic = '{}AlbumWinners{}{}'.format(prefix, self._DELIMITER, op)
        buffer = TMemoryOutputBuffer(self._transport.get_publish_size_limit())
