...
type FStore interface {
	...
	SomeUnionThing(ctx frugal.FContext, some_union *AUnion) (r *AUnion, err error)
}
...
func NewFStoreClient(provider *frugal.FServiceProvider, middleware ...frugal.ServiceMiddleware) *FStoreClient {
	...
	methods["some_union_thing"] = frugal.NewMethod(client, client.some_union_thing, "some_union_thing", middleware)
	return client
}
...
func (f *FStoreClient) SomeUnionThing(ctx frugal.FContext, some_union *AUnion) (r *AUnion, err error) {
	ret := f.methods["some_union_thing"].Invoke([]interface{}{ctx, some_union})
	if len(ret) != 2 {
		panic(fmt.Sprintf("Middleware returned %d arguments, expected 2", len(ret)))
	}
	r = ret[0].(*AUnion)
...
func (f *FStoreClient) some_union_thing(ctx frugal.FContext, some_union *AUnion) (r *AUnion, err error) {
	buffer := frugal.NewTMemoryOutputBuffer(f.transport.GetRequestSizeLimit())
	oprot := f.protocolFactory.GetProtocol(buffer)
	if err = oprot.WriteRequestHeader(ctx); err != nil {
		return
	}
	if err = oprot.WriteMessageBegin("some_union_thing", thrift.CALL, 0); err != nil {
		return
	}
	args := StoreSomeUnionThingArgs{
	...
	if method != "some_union_thing" {
		err = thrift.NewTApplicationException(frugal.APPLICATION_EXCEPTION_WRONG_METHOD_NAME, "some_union_thing failed: wrong method name")
		return
	}
	if mTypeId == thrift.EXCEPTION {
		error0 := thrift.NewTApplicationException(frugal.APPLICATION_EXCEPTION_UNKNOWN, "Unknown Exception")
	...
	if mTypeId != thrift.REPLY {
		err = thrift.NewTApplicationException(frugal.APPLICATION_EXCEPTION_INVALID_MESSAGE_TYPE, "some_union_thing failed: invalid message type")
		return
	}
	result := StoreSomeUnionThingResult{}
	if err = result.Read(iprot); err != nil {
		return
	}
	if err = iprot.ReadMessageEnd(); err != nil {
...
func NewFStoreProcessor(handler FStore, middleware ...frugal.ServiceMiddleware) *FStoreProcessor {
	...
	p.AddToProcessorMap("some_union_thing", &storeFSomeUnionThing{frugal.NewFBaseProcessorFunction(p.GetWriteMutex(), frugal.NewMethod(handler, handler.SomeUnionThing, "SomeUnionThing", middleware))})
	return p
}
...
type storeFSomeUnionThing struct {
	*frugal.FBaseProcessorFunction
}

func (p *storeFSomeUnionThing) Process(ctx frugal.FContext, iprot, oprot *frugal.FProtocol) error {
	args := StoreSomeUnionThingArgs{}
	var err error
	if err = args.Read(iprot); err != nil {
		iprot.ReadMessageEnd()
		p.GetWriteMutex().Lock()
		err = storeWriteApplicationError(ctx, oprot, frugal.APPLICATION_EXCEPTION_PROTOCOL_ERROR, "some_union_thing", err.Error())
		p.GetWriteMutex().Unlock()
		return err
	}
	...
	result := StoreSomeUnionThingResult{}
	var err2 error
	ret := p.InvokeMethod([]interface{}{ctx, args.SomeUnion})
	if len(ret) != 2 {
		panic(fmt.Sprintf("Middleware returned %d arguments, expected 2", len(ret)))
	}
	if ret[1] != nil {
	...
	if err2 != nil {
		if err3, ok := err2.(thrift.TApplicationException); ok {
			p.GetWriteMutex().Lock()
			oprot.WriteResponseHeader(ctx)
			oprot.WriteMessageBegin("some_union_thing", thrift.EXCEPTION, 0)
			err3.Write(oprot)
			oprot.WriteMessageEnd()
			oprot.Flush()
			p.GetWriteMutex().Unlock()
		...
		p.GetWriteMutex().Lock()
		err2 := storeWriteApplicationError(ctx, oprot, frugal.APPLICATION_EXCEPTION_INTERNAL_ERROR, "some_union_thing", "Internal error processing some_union_thing: "+err2.Error())
		p.GetWriteMutex().Unlock()
		return err2
	} else {
		var retval *AUnion = ret[0].(*AUnion)
		result.Success = retval
	}
	p.GetWriteMutex().Lock()
	defer p.GetWriteMutex().Unlock()
	if err2 = oprot.WriteResponseHeader(ctx); err2 != nil {
		if frugal.IsErrTooLarge(err2) {
			storeWriteApplicationError(ctx, oprot, frugal.APPLICATION_EXCEPTION_RESPONSE_TOO_LARGE, "some_union_thing", err2.Error())
			return nil
		}
		err = err2
	}
	if err2 = oprot.WriteMessageBegin("some_union_thing", thrift.REPLY, 0); err2 != nil {
		if frugal.IsErrTooLarge(err2) {
			storeWriteApplicationError(ctx, oprot, frugal.APPLICATION_EXCEPTION_RESPONSE_TOO_LARGE, "some_union_thing", err2.Error())
			return nil
		}
...
type StoreSomeUnionThingArgs struct {
	SomeUnion *AUnion `thrift:"some_union,1" db:"some_union" json:"some_union"`
}

func NewStoreSomeUnionThingArgs() *StoreSomeUnionThingArgs {
	return &StoreSomeUnionThingArgs{}
}

var StoreSomeUnionThingArgs_SomeUnion_DEFAULT *AUnion

func (p *StoreSomeUnionThingArgs) IsSetSomeUnion() bool {
	return p.SomeUnion != nil
}

func (p *StoreSomeUnionThingArgs) GetSomeUnion() *AUnion {
	if !p.IsSetSomeUnion() {
		return StoreSomeUnionThingArgs_SomeUnion_DEFAULT
	}
	return p.SomeUnion
}

func (p *StoreSomeUnionThingArgs) Read(iprot thrift.TProtocol) error {
	if _, err := iprot.ReadStructBegin(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T read error: ", p), err)
	}
...
func (p *StoreSomeUnionThingArgs) ReadField1(iprot thrift.TProtocol) error {
	p.SomeUnion = NewAUnion()
	if err := p.SomeUnion.Read(iprot); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T error reading struct: ", p.SomeUnion), err)
	}
	return nil
}

func (p *StoreSomeUnionThingArgs) Write(oprot thrift.TProtocol) error {
	if err := oprot.WriteStructBegin("some_union_thing_args"); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
	}
	if err := p.writeField1(oprot); err != nil {
		return err
...
func (p *StoreSomeUnionThingArgs) writeField1(oprot thrift.TProtocol) error {
	if err := oprot.WriteFieldBegin("some_union", thrift.STRUCT, 1); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field begin error 1:some_union: ", p), err)
	}
	if err := p.SomeUnion.Write(oprot); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T error writing struct: ", p.SomeUnion), err)
	}
	if err := oprot.WriteFieldEnd(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field end error 1:some_union: ", p), err)
	}
	return nil
}

func (p *StoreSomeUnionThingArgs) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("StoreSomeUnionThingArgs(%+v)", *p)
}

type StoreSomeUnionThingResult struct {
	Success *AUnion `thrift:"success,0" db:"success" json:"success,omitempty"`
}

func NewStoreSomeUnionThingResult() *StoreSomeUnionThingResult {
	return &StoreSomeUnionThingResult{}
}

var StoreSomeUnionThingResult_Success_DEFAULT *AUnion

func (p *StoreSomeUnionThingResult) IsSetSuccess() bool {
	return p.Success != nil
}

func (p *StoreSomeUnionThingResult) GetSuccess() *AUnion {
	if !p.IsSetSuccess() {
		return StoreSomeUnionThingResult_Success_DEFAULT
	}
	return p.Success
}

func (p *StoreSomeUnionThingResult) Read(iprot thrift.TProtocol) error {
	if _, err := iprot.ReadStructBegin(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T read error: ", p), err)
	}
...
func (p *StoreSomeUnionThingResult) ReadField0(iprot thrift.TProtocol) error {
	p.Success = NewAUnion()
	if err := p.Success.Read(iprot); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T error reading struct: ", p.Success), err)
	}
...
func (p *StoreSomeUnionThingResult) Write(oprot thrift.TProtocol) error {
	if err := oprot.WriteStructBegin("some_union_thing_result"); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write struct begin error: ", p), err)
	}
	if err := p.writeField0(oprot); err != nil {
		return err
...
func (p *StoreSomeUnionThingResult) writeField0(oprot thrift.TProtocol) error {
	if p.IsSetSuccess() {
		if err := oprot.WriteFieldBegin("success", thrift.STRUCT, 0); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field begin error 0:success: ", p), err)
		}
...
func (p *StoreSomeUnionThingResult) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("StoreSomeUnionThingResult(%+v)", *p)
}
